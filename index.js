...
const defaultConfig = require('./i18n-rainbow.config')
const transformRawLocales = require('./config/transform-raw-locales')
const generateI18nextScannerConfig = require('./config/i18next-scanner.config')
const diff = require('./config/diff-increment')
const translateExcel = require('./config/translate-excel')

function getConfig (configPath = 'i18n-rainbow.config.js') {
  const config = require(path.resolve(configPath))

  if (!config) {
    throw new Error('未找到 i18n-rainbow.config.js 文件')
  }
...
program
.command('scan')
.option('-c <config>', '指定配置信息，默认为 ./i18n-rainbow.config.js')
...
console.log(i18nextScannerConfig.input)
